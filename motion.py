...
def augment(sequence,
    ...
    if enable_global_scale:
        ...
        min_scale = max(min_scale, min_diam/typical_diam)
        max_scale = min(max_scale, max_diam/typical_diam)
        if keep_inside:
            # Want: all(scale * extent_size <= 1)
            # Equivalent: scale <= min(1 / extent_size)
            max_scale = min(max_scale, min(1./tmp_extent_size))
        if not min_scale <= max_scale:
            # Could not find scale to satisfy requirements.
            # return None
            return sequence
        init_scale = math.exp(rand.uniform(math.log(min_scale), math.log(max_scale)))
    ...
    # new_viewport/canvas = crop(new_viewport/old_viewport, inv(old_viewport/canvas))
    output['viewports'] = geom_np.crop_rect(viewport, geom_np.crop_inverse(sequence['viewports']))
    # rect/new_viewport = crop(rect/old_viewport, new_viewport/old_viewport)
