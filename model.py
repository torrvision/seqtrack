...
class RNN_dual_mix_reduce(object):
    '''
    ...
    def _load_model(self, inputs, o):
        ...
        def pass_cnn_a(x):
            with slim.arg_scope([slim.conv2d],
                ...
                x = slim.conv2d(x, 128, [3, 3], stride=1, scope='conv4')
                x = slim.max_pool2d(x, 2, scope='pool3')
                x = slim.conv2d(x, 256, [3, 3], stride=1, scope='conv5')
                x = slim.flatten(x)
        ...
        def pass_single_level_cross_correlation(search, filt):
            ''' depth-wise convolution with 1x1 filter -> multiplication.
            '''
            with slim.arg_scope([slim.fully_connected],
                    weights_regularizer=slim.l2_regularizer(o.wd)):
                depth = search.shape.as_list()[-1]
                scoremap = search * tf.expand_dims(tf.expand_dims(slim.fully_connected(filt, depth), 1), 1)
            return scoremap

        def pass_single_level_deconvolution(x):
            ''' single-level deconvolution.
            However, it performs multiple consecutive deconvolutions.
            '''
            with slim.arg_scope([slim.conv2d],
                    kernel_size=[3,3],
                    weights_regularizer=slim.l2_regularizer(o.wd)):
                x = slim.conv2d(tf.image.resize_images(x, [20, 20]), 64)
                x = slim.conv2d(tf.image.resize_images(x, [41, 41]), 32)
                x = slim.conv2d(tf.image.resize_images(x, [81, 81]), 16)
            return x
        ...
        if self.feed_examplar:
            with tf.variable_scope('cnn-e'): # NOTE: no parameter sharing with `cnn2` yet.
                examplar = pass_cnn_a(concat([x0, get_masks_from_rectangles(y0, o)], axis=3))
        ...
        for t in range(o.ntimesteps):
            ...
            with tf.variable_scope('cnn1', reuse=(t > 0)):
                cnn1out = pass_cnn_s(x_curr)

            #with tf.variable_scope('cnn2', reuse=True if self.feed_examplar else (t > 0)): # parameter sharing with `CNN-Examplar`
            with tf.variable_scope('cnn2', reuse=(t > 0)):
                #xy = tf.stop_gradient(concat([x_prev, hmap_prev], axis=3))
                xy = concat([x_prev, hmap_prev], axis=3)
                cnn2out = pass_cnn_a(xy)
            ...
            with tf.variable_scope('single_level_cross_correlation', reuse=(t > 0)):
                scoremap = pass_single_level_cross_correlation(cnn1out, xin)

            with tf.variable_scope('single_level_deconvolution', reuse=(t > 0)):
                scoremap = pass_single_level_deconvolution(scoremap)
