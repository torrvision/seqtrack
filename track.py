...
def main():
    ...
    sequence = {}
    sequence['image_files'] = [args.image_format % i for i in range(args.start, args.end+1)]
    sequence['viewports'] = [geom_np.unit_rect() for _ in range(args.start, args.end+1)]
    init_rect = np.asfarray([args.init_rect[k] for k in ['xmin', 'ymin', 'xmax', 'ymax']])
    sequence['labels'] = [init_rect if i == args.start else geom_np.unit_rect()
        for i in range(args.start, args.end+1)]
    sequence['label_is_valid'] = [True]
