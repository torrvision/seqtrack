...
class SiamFCSeq(models_interface.IterModel):
    ...
    def __init__(
        ...
        self._rsp_img_ref_size = rsp_img_ref_size
    ...
    def start(self, frame, aspect, run_opts, enable_loss,
        ...
        with tf.name_scope(name) as scope:
            ...
            cell_size = [tf.shape(template_feat)[0], 3, 3, self._encoder_dims]
            ...
            state = {
                ...
                'encoder':       map_nested(tf.identity, encoder_state_init),
                #'decoder':       map_nested(tf.identity, decoder_state_init),
            }
            return state
    ...
    def next(self, frame, prev_state, name='timestep'):
        with tf.name_scope(name) as scope:
            ...
            rsp_siamfc = util.crop(
                ...
                [self._rsp_img_ref_size]*2,
                pad_value=tf.reduce_min(response, (-3,-2,-1), keep_dims=True),
                feather=True)

            # NOTE: Pre-processing siamfc response before encoder?
            ...
            # In case max, it can be used to select response map!
            # Then, it better be respected when computing translation as well.
            #_, scale = util.find_peak_pyr(rsp_siamfc, scales,
            #                              is_pyramid=False,
            #                              eps_rel=self._arg_max_eps_rel)
            #scale_tile = tf.tile(tf.expand_dims(scale, -1), [1,tf.shape(scales)[0]])
            #max_scale_ind = tf.argmin(tf.abs(scale_tile - scales), axis=1)
            #tf.slice(rsp_siamfc, [0, max_scale_ind, 0, 0, 0], [-1, 1, -1, -1, -1])

            # Encoder-decoder
            with tf.variable_scope('encoder_decoder', reuse=(self._num_frames > 0)):
                rsp_decoder, encoder_state = _encoder_decoder(
                    rsp_siamfc[:, mid_scale], prev_state['encoder'],
                    tanh_summary=False, is_training=self._is_training)
            assert all(np.array(rsp_decoder.shape.as_list()[-3:-1]) == [self._rsp_img_ref_size]*2)

            # loss. # TODO: GT global responses are currentyl very different..
            if self._enable_loss and self._enable_motion_loss:
                centers = util.make_grid_centers([self._rsp_img_ref_size]*2)
                ...
                loss = tf.nn.sigmoid_cross_entropy_with_logits(
                    labels=labels, logits=tf.squeeze(rsp_decoder, -1))
            ...
            rsp_decoder = tf.sigmoid(rsp_decoder)
            translation, _ = util.find_peak_pyr(tf.expand_dims(rsp_decoder, 1),
            ...
            overlay = tf.image.resize_images(util.colormap(rsp_decoder, _COLORMAP),
                                             frame['x'].shape.as_list()[-3:-1], align_corners=True)
...
def _encoder_decoder(x, rnn_state, enable_hglass=True, residual_type='add',
    ...
    with tf.name_scope(name) as scope:
        ...
        # RNN
        if enable_rnn:
            x, rnn_state = _conv_lstm(x, rnn_state, enable_rnn_bnorm, is_training)
        else:
            x = slim.conv2d(x, rnn_state['h'].shape.as_list()[-1], 3, 1, scope='rnn_replace')
        # Optional tanh for summary; following the paper..
        if tanh_summary:
            x = slim.conv2d(x, x.shape.as_list()[-1], 1, 1,
                            normalizer_fn=slim.batch_norm,
                            normalizer_params={'is_training': is_training},
                            activation_fn=tf.nn.tanh, scope='context')
        # Decoder
        with slim.arg_scope([slim.conv2d], **bnorm_args):
            if enable_hglass: # TODO: test more skip connections
                ...
                x = slim.conv2d(resz_imgs(x, [17, 17]), 32, 3, 1, scope='dec3')
                x = slim.conv2d(resz_imgs(x, [33, 33]), 1, 3, 1,
                                activation_fn=None, normalizer_fn=None, scope='dec4')
        return x, rnn_state
