...
class SiamFC(models_interface.IterModel):
    ...
    def start(self, frame, aspect, run_opts, enable_loss,
        ...
        with tf.name_scope(name) as scope:
            ...
            with tf.variable_scope('feature_net', reuse=False):
                template_feat, rfs, self._moments = self._feature_net(template_input, rfs, moments=None)
    ...
    def next(self, frame, prev_state, name='timestep'):
        with tf.name_scope(name) as scope:
            ...
            with tf.variable_scope('feature_net', reuse=True):
                search_feat, rfs, _ = self._feature_net(search_input, rfs, moments=self._moments)
    ...
    def _feature_net(self, x, rfs, moments):
        return _feature_net(
            x, rfs, moments, padding=self._feature_padding, arch=self._feature_arch,
...
def _feature_net(
        x, rfs=None, moments=None, padding=None,
    ...
    if len(x.shape) > 4:
        ...
        x, rfs, batch_moments = _feature_net(
            x, rfs=rfs, moments=moments, padding=padding, arch=arch, output_act=output_act,
            enable_bnorm=enable_bnorm, is_training=is_training, name=name)
        x = restore(x, 0)
        return x, rfs, batch_moments
    ...
    with tf.name_scope(name) as scope:
        ...
        with slim.arg_scope([slim.conv2d], **args):
            if arch == 'alexnet':
                ...
                x, rfs, batch_moments['conv1_mean'], batch_moments['conv1_variance'] = _conv2d(
                    x, rfs, 96, [11, 11], 2, padding=padding, is_training=is_training, scope='conv1',
                    mean=moments.get('conv1_mean', None), variance=moments.get('conv1_variance', None))
                x, rfs = util.max_pool2d_rf(x, rfs, [3, 3], 2, padding=padding, scope='pool1')
                x, rfs, batch_moments['conv2_mean'], batch_moments['conv2_variance'] = _conv2d(
                    x, rfs, 256, [5, 5], padding=padding, is_training=is_training, scope='conv2',
                    mean=moments.get('conv2_mean', None), variance=moments.get('conv2_variance', None))
                x, rfs = util.max_pool2d_rf(x, rfs, [3, 3], 2, padding=padding, scope='pool2')
                x, rfs, batch_moments['conv3_mean'], batch_moments['conv3_variance'] = _conv2d(
                    x, rfs, 384, [3, 3], padding=padding, is_training=is_training, scope='conv3',
                    mean=moments.get('conv3_mean', None), variance=moments.get('conv3_variance', None))
                x, rfs, batch_moments['conv4_mean'], batch_moments['conv4_variance'] = _conv2d(
                    x, rfs, 384, [3, 3], padding=padding, is_training=is_training, scope='conv4',
                    mean=moments.get('conv4_mean', None), variance=moments.get('conv4_variance', None))
                x, rfs = util.conv2d_rf(x, rfs, 256, [3, 3], padding=padding, scope='conv5',
                                        activation_fn=get_act(output_act), normalizer_fn=None)
            elif arch == 'darknet':
...
def _conv2d(x, rfs, num_outputs, kernel_size, stride=1, padding='SAME',
            mean=None, variance=None, is_training=None, scope='conv', **kwargs):
    x, rfs = util.conv2d_rf(x, rfs, num_outputs, kernel_size, stride,
                            normalizer_fn=None, activation_fn=None,
                            padding=padding, scope=scope)
    x, mean, variance = _batch_norm(x, mean, variance, is_training=is_training)
    x = tf.nn.relu(x)
    return x, rfs, mean, variance

def _batch_norm(x, mean, variance, is_training=None):
    assert is_training is not None
    x, mean, variance = shared_bnorm.batch_norm(x, mean, variance, is_training=is_training)
